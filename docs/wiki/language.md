...
The features marked as implemented below are supported by the current parser and by `hlir`.

### Parsed or designed, but not fully implemented in `hlir`
...
- `while` statements parse, but are rejected in `hlir`
...
Parsed but not fully implemented in `hlir`:
...
This syntax parses, but `while` is not fully implemented in `hlir` yet.
...
Only the arithmetic and comparison forms here are fully implemented today. Dereference and indexing syntax exist, but are not fully implemented in `hlir`.
