...
namespace benson
{
  ...
  Parser::parse_expression(int current_precedence)
  {
    auto expr = [&]() -> std::unique_ptr<ast::Expression>
    {
      ...
      if (_reader->peek().token == Token::lbracket &&
      ...
      for (;;)
      {
        if (auto const postfix_op =
            ...
            postfix_op && ast::get_operator_precedence(*postfix_op) <=
                            current_precedence)
        {
          ...
          else
          {
            ...
            primary =
              std::make_unique<ast::Expression>(std::move(postfix));
          }
        }
        else
        {
    ...
    for (;;)
    {
      if (auto const bin_op =
            ast::get_binary_operator(_reader->peek().token);
          bin_op &&
          ast::get_operator_precedence(*bin_op) <= current_precedence)
      {
  ...
  {
    return ast::Int_literal_expression{
      .literal = expect(Token::int_literal)
    };
  }
  ...
  {
    return ast::Identifier_expression{
      .identifier = expect(Token::identifier)
    };
  }
