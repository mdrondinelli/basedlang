...
{
  kalehlir::Type_pool types;
  kalehlir::Compilation_context compiler;
...
kalehlir::Constant_value evaluate_constant(
  kalehlir::Compilation_context &compiler,
...
kalehlir::Type *
compile_type(kalehlir::Compilation_context &compiler, std::string_view source)
{
  auto parse_fixture = Parse_fixture{std::string{source}};
  auto const expr = parse_fixture.parser.parse_expression();
...
{
  auto const value = kalehlir::validate_int_literal(
    "42",
...
TEST_CASE("compile_type_expression - sized array")
{
  ...
  auto const sa = std::get_if<kalehlir::Sized_array_type>(&type->data);
  REQUIRE(sa != nullptr);
...
std::pair<kalehlir::Type_pool, kalehlir::Translation_unit>
compile_program(std::string source)
{
  ...
  auto tu = kalehlir::compile(ast, &types);
...
TEST_CASE("compile - function returning literal")
{
  ...
  auto const result = kalehlir::interpret(f, {});
...
TEST_CASE("compile - function with parameter")
{
  ...
  auto const args = std::array<kalehlir::Constant_value, 1>{std::int32_t{42}};
  auto const result = kalehlir::interpret(f, args);
  REQUIRE(std::holds_alternative<std::int32_t>(result));
...
TEST_CASE("compile - function with arithmetic")
{
  ...
  REQUIRE(tu.functions.size() == 1);
  auto const args = std::array<kalehlir::Constant_value, 1>{std::int32_t{41}};
  auto const result = kalehlir::interpret(*tu.functions[0], args);
  REQUIRE(std::holds_alternative<std::int32_t>(result));
  CHECK(std::get<std::int32_t>(result) == 42);
}
...
TEST_CASE("compile - function with multiple parameters")
{
  ...
  REQUIRE(tu.functions.size() == 1);
  auto const args =
    std::array<kalehlir::Constant_value, 2>{std::int32_t{1}, std::int32_t{2}};
  auto const result = kalehlir::interpret(*tu.functions[0], args);
  REQUIRE(std::holds_alternative<std::int32_t>(result));
...
TEST_CASE("compile - function with if expression")
{
  ...
  REQUIRE(tu.functions.size() == 1);
  auto const &f = *tu.functions[0];
  auto const args1 = std::array<kalehlir::Constant_value, 1>{std::int32_t{-5}};
  auto const r1 = kalehlir::interpret(f, args1);
  CHECK(std::get<std::int32_t>(r1) == 5);
  auto const args2 = std::array<kalehlir::Constant_value, 1>{std::int32_t{3}};
  auto const r2 = kalehlir::interpret(f, args2);
...
TEST_CASE("compile - function with block and let bindings")
{
  ...
  REQUIRE(tu.functions.size() == 1);
  auto const result = kalehlir::interpret(*tu.functions[0], {});
  REQUIRE(std::holds_alternative<std::int32_t>(result));
...
TEST_CASE("compile - function calling another function")
{
  ...
  auto const result = kalehlir::interpret(*tu.functions[1], {});
  REQUIRE(std::holds_alternative<std::int32_t>(result));
  CHECK(std::get<std::int32_t>(result) == 42);
}
...
TEST_CASE("compile - nested function calls")
{
  ...
  auto const result = kalehlir::interpret(*tu.functions[2], {});
  REQUIRE(std::holds_alternative<std::int32_t>(result));
  CHECK(std::get<std::int32_t>(result) == 42);
}
...
TEST_CASE("compile - recursive function")
{
  ...
  REQUIRE(tu.functions.size() == 1);
  auto const args = std::array<kalehlir::Constant_value, 1>{std::int32_t{10}};
  auto const result = kalehlir::interpret(*tu.functions[0], args);
  REQUIRE(std::holds_alternative<std::int32_t>(result));
...
TEST_CASE("compile - recurse keyword")
{
  ...
  REQUIRE(tu.functions.size() == 1);
  auto const args = std::array<kalehlir::Constant_value, 1>{std::int32_t{5}};
  auto const result = kalehlir::interpret(*tu.functions[0], args);
  REQUIRE(std::holds_alternative<std::int32_t>(result));
...
TEST_CASE("compile - fuel exhaustion")
{
  ...
  REQUIRE(tu.functions.size() == 1);
  auto const args = std::array<kalehlir::Constant_value, 1>{std::int32_t{0}};
  auto fuel = std::int32_t{100};
  CHECK_THROWS_AS(
    kalehlir::interpret(*tu.functions[0], args, fuel),
    kalehlir::Fuel_exhausted_error
  );
}

TEST_CASE("Fuel_exhausted_error - std::exception contract")
{
  auto const error = kalehlir::Fuel_exhausted_error{};
...
TEST_CASE("compile - void if expression")
{
  ...
  auto const args = std::array<kalehlir::Constant_value, 1>{std::int32_t{-5}};
  auto const result = kalehlir::interpret(*tu.functions[0], args);
  CHECK(std::holds_alternative<kalehlir::Void_value>(result));
}
...
TEST_CASE("compile - if expression in tail position")
{
  ...
  REQUIRE(tu.functions.size() == 1);
  auto const args1 = std::array<kalehlir::Constant_value, 1>{std::int32_t{-3}};
  auto const r1 = kalehlir::interpret(*tu.functions[0], args1);
  REQUIRE(std::holds_alternative<std::int32_t>(r1));
  CHECK(std::get<std::int32_t>(r1) == 0);
  auto const args2 = std::array<kalehlir::Constant_value, 1>{std::int32_t{7}};
  auto const r2 = kalehlir::interpret(*tu.functions[0], args2);
...
TEST_CASE("compile - nested if expressions")
{
  ...
  REQUIRE(tu.functions.size() == 1);
  auto const &f = *tu.functions[0];
  auto const args1 = std::array<kalehlir::Constant_value, 2>{
    std::int32_t{-1},
    std::int32_t{-1}
  };
  CHECK(std::get<std::int32_t>(kalehlir::interpret(f, args1)) == 1);
  auto const args2 =
    std::array<kalehlir::Constant_value, 2>{std::int32_t{-1}, std::int32_t{1}};
  CHECK(std::get<std::int32_t>(kalehlir::interpret(f, args2)) == 2);
  auto const args3 =
    std::array<kalehlir::Constant_value, 2>{std::int32_t{1}, std::int32_t{0}};
  CHECK(std::get<std::int32_t>(kalehlir::interpret(f, args3)) == 3);
}
...
TEST_CASE("compile - multiple else-if branches")
{
  ...
  REQUIRE(tu.functions.size() == 1);
  auto const &f = *tu.functions[0];
  auto const a1 = std::array<kalehlir::Constant_value, 1>{std::int32_t{1}};
  CHECK(std::get<std::int32_t>(kalehlir::interpret(f, a1)) == 10);
  auto const a2 = std::array<kalehlir::Constant_value, 1>{std::int32_t{2}};
  CHECK(std::get<std::int32_t>(kalehlir::interpret(f, a2)) == 20);
  auto const a3 = std::array<kalehlir::Constant_value, 1>{std::int32_t{3}};
  CHECK(std::get<std::int32_t>(kalehlir::interpret(f, a3)) == 30);
  auto const a4 = std::array<kalehlir::Constant_value, 1>{std::int32_t{99}};
  CHECK(std::get<std::int32_t>(kalehlir::interpret(f, a4)) == 0);
}
...
TEST_CASE("compile - function with Float64 parameter and arithmetic")
{
  ...
  REQUIRE(tu.functions.size() == 1);
  auto const args = std::array<kalehlir::Constant_value, 1>{double{3.5}};
  auto const result = kalehlir::interpret(*tu.functions[0], args);
  REQUIRE(std::holds_alternative<double>(result));
...
TEST_CASE("compile - function with Float64 comparison")
{
  ...
  REQUIRE(tu.functions.size() == 1);
  auto const &f = *tu.functions[0];
  auto const args1 = std::array<kalehlir::Constant_value, 1>{double{1.0}};
  CHECK(std::get<bool>(kalehlir::interpret(f, args1)) == true);
  auto const args2 = std::array<kalehlir::Constant_value, 1>{double{-1.0}};
  CHECK(std::get<bool>(kalehlir::interpret(f, args2)) == false);
}
