...
TEST_CASE("Parser - first.benson produces a declaration")
{
  ...
  CHECK(decl.eq.text == "=");
  auto const fn = std::get_if<benson::ast::Fn_expression>(&decl.initializer.value);
  REQUIRE(fn != nullptr);
...
TEST_CASE("Parser - parameters.benson parses successfully")
{
  ...
  auto const first_param0_type = std::get_if<benson::ast::Identifier_expression>(
  ...
  auto const first_param1_type = std::get_if<benson::ast::Identifier_expression>(
  ...
  auto const first_ret =
    std::get_if<benson::ast::Return_statement>(&first_body->statements[0].value);
...
TEST_CASE("parse_let_statement - function declaration")
{
  ...
  CHECK(stmt.eq.text == "=");
  auto const fn = std::get_if<benson::ast::Fn_expression>(&stmt.initializer.value);
  REQUIRE(fn != nullptr);
  CHECK(fn->kw_fn.text == "fn");
  REQUIRE(fn->body != nullptr);
  auto const body = std::get_if<benson::ast::Block_expression>(&fn->body->value);
...
TEST_CASE("parse_block_expression")
{
  ...
  CHECK(
    std::get_if<benson::ast::Let_statement>(&block.statements[1].value) != nullptr
  );
...
TEST_CASE("parse_block_expression - statements then tail")
{
  ...
  CHECK(
    std::get_if<benson::ast::Let_statement>(&block.statements[0].value) != nullptr
  );
  REQUIRE(block.tail != nullptr);
  auto const bin = std::get_if<benson::ast::Binary_expression>(&block.tail->value);
  REQUIRE(bin != nullptr);
  CHECK(bin->op.text == "+");
}
...
TEST_CASE("parse_primary_expression - dispatches to int literal")
{
  ...
  CHECK(std::get_if<benson::ast::Int_literal_expression>(&expr->value) != nullptr);
}
...
TEST_CASE("parse_primary_expression - dispatches to identifier")
{
  auto fixture = Parse_fixture{"x"};
  auto const expr = fixture.parser.parse_primary_expression();
  CHECK(std::get_if<benson::ast::Identifier_expression>(&expr->value) != nullptr);
}
...
TEST_CASE("parse_expression - multiplicative before additive")
{
  ...
  REQUIRE(add != nullptr);
  CHECK(add->op.text == "+");
  auto const mul = std::get_if<benson::ast::Binary_expression>(&add->right->value);
  REQUIRE(mul != nullptr);
  CHECK(mul->op.text == "*");
  auto const two =
...
TEST_CASE("parse_expression - all operators")
{
  ...
  CHECK(sub->op.text == "-");
  // left of -: -f() + +2
  auto const add = std::get_if<benson::ast::Binary_expression>(&sub->left->value);
  REQUIRE(add != nullptr);
  CHECK(add->op.text == "+");
  // left of +: -f()
  ...
  // right of -: ((3 * 4) / 5) % 6
  auto const mod = std::get_if<benson::ast::Binary_expression>(&sub->right->value);
  REQUIRE(mod != nullptr);
  CHECK(mod->op.text == "%");
  // left of %: (3 * 4) / 5
  auto const div = std::get_if<benson::ast::Binary_expression>(&mod->left->value);
  REQUIRE(div != nullptr);
  CHECK(div->op.text == "/");
  // left of /: 3 * 4
  auto const mul = std::get_if<benson::ast::Binary_expression>(&div->left->value);
  REQUIRE(mul != nullptr);
  CHECK(mul->op.text == "*");
}
...
TEST_CASE("parse_expression - call binds tighter than binary op")
{
  ...
  REQUIRE(add != nullptr);
  CHECK(add->op.text == "+");
  CHECK(std::get_if<benson::ast::Call_expression>(&add->left->value) != nullptr);
}
...
TEST_CASE("parse_fn_expression - array parameter")
{
  ...
  auto const elem =
    std::get_if<benson::ast::Identifier_expression>(&param_type->operand->value);
...
TEST_CASE("parse_expression - index: binds tighter than binary op")
{
  ...
  REQUIRE(add != nullptr);
  CHECK(add->op.text == "+");
  CHECK(std::get_if<benson::ast::Index_expression>(&add->left->value) != nullptr);
}
...
TEST_CASE("parse_expression - dereference")
{
  ...
  auto const expr = fixture.parser.parse_expression();
  auto const postfix = std::get_if<benson::ast::Postfix_expression>(&expr->value);
...
TEST_CASE("parse_expression - dereference: index then deref")
{
  ...
  CHECK(postfix->op.text == "^");
  CHECK(
    std::get_if<benson::ast::Index_expression>(&postfix->operand->value) != nullptr
  );
}
...
TEST_CASE("parse_expression - address-of: postfix binds tighter")
{
  ...
  CHECK(unary->op.text == "&");
  CHECK(
    std::get_if<benson::ast::Index_expression>(&unary->operand->value) != nullptr
  );
}
...
TEST_CASE("parse_expression - address-of: in binary expression")
{
  ...
  REQUIRE(add != nullptr);
  CHECK(add->op.text == "+");
  auto const left = std::get_if<benson::ast::Prefix_expression>(&add->left->value);
  REQUIRE(left != nullptr);
  CHECK(left->op.text == "&");
  auto const right =
    std::get_if<benson::ast::Prefix_expression>(&add->right->value);
...
TEST_CASE("parse_expression - additive before comparison")
{
  ...
  REQUIRE(lt != nullptr);
  CHECK(lt->op.text == "<");
  auto const left = std::get_if<benson::ast::Binary_expression>(&lt->left->value);
  REQUIRE(left != nullptr);
  CHECK(left->op.text == "+");
  auto const right =
...
TEST_CASE("parse_expression - comparison before equality")
{
  ...
  REQUIRE(eq != nullptr);
  CHECK(eq->op.text == "==");
  auto const left = std::get_if<benson::ast::Binary_expression>(&eq->left->value);
  REQUIRE(left != nullptr);
  CHECK(left->op.text == "<");
  auto const right =
...
TEST_CASE("parse_statement - while: simple")
{
  ...
  auto const stmt = fixture.parser.parse_statement();
  auto const while_stmt = std::get_if<benson::ast::While_statement>(&stmt.value);
  REQUIRE(while_stmt != nullptr);
  CHECK(while_stmt->kw_while.text == "while");
  auto const cond =
    std::get_if<benson::ast::Identifier_expression>(&while_stmt->condition->value);
  REQUIRE(cond != nullptr);
  CHECK(cond->identifier.text == "x");
