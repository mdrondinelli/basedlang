...
This module consumes lexemes and produces the AST defined in [`ast`](./ast.md).

## Core abstractions

### `Parser`

Hand-written parser over `Lexeme_stream_reader`. Produces `Translation_unit`, `Expression`, and `Statement` values from `ast`.
...
- [`ast`](./ast.md) — data model produced by this module
