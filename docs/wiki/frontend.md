...
produces executable HLIR (the [`hlir`](./hlir.md) data model).
...
through the `benson::hlir::` qualifier.
...
- an `hlir::Constant_value` (compile-time value)
...
3. types are canonicalized through `hlir::Type_pool`
...
`compile_expression` returns an `hlir::Operand` — either a `Register`
(runtime result) or an `hlir::Constant_value` (compile-time result).
...
mutability) or a compile-time `hlir::Constant_value`.

## What to keep stable

- type identity must come from `hlir::Type_pool`
