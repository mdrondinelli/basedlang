...
`bensonparse` is the parser module.
...
This module consumes lexemes and produces the AST defined in [`bensonast`](./bensonast.md).

## Core abstractions

### `Parser`

Hand-written parser over `Lexeme_stream_reader`. Produces `Translation_unit`, `Expression`, and `Statement` values from `bensonast`.
...
- [`bensonast`](./bensonast.md) — data model produced by this module
