...
struct Parse_fixture
{
  ...
  kalelex::Istream_binary_stream binary_stream;
  kalelex::Utf8_char_stream char_stream;
  kalelex::Lexeme_stream lexeme_stream;
  kalelex::Lexeme_stream_reader reader;
  kaleparse::Parser parser;
...
TEST_CASE("Parser - first.kale produces a declaration")
{
  auto file = std::ifstream{std::string{EXAMPLES_PATH} + "/first.kale"};
  auto binary_stream = kalelex::Istream_binary_stream{&file};
  auto char_stream = kalelex::Utf8_char_stream{&binary_stream};
  auto lexeme_stream = kalelex::Lexeme_stream{&char_stream};
  auto reader = kalelex::Lexeme_stream_reader{&lexeme_stream};
  auto parser = kaleparse::Parser{&reader};
  ...
  CHECK(decl.eq.text == "=");
  auto const fn = std::get_if<kaleast::Fn_expression>(&decl.initializer.value);
  REQUIRE(fn != nullptr);
  CHECK(fn->kw_fn.text == "fn");
  CHECK(fn->lparen.text == "(");
  CHECK(fn->rparen.text == ")");
  CHECK(fn->return_type_specifier.colon.text == ":");
  auto const return_type = std::get_if<kaleast::Identifier_expression>(
  ...
  auto const body = std::get_if<kaleast::Block_expression>(&fn->body->value);
  REQUIRE(body != nullptr);
  CHECK(body->lbrace.text == "{");
  REQUIRE(body->statements.size() == 1);
  auto const ret_stmt =
    std::get_if<kaleast::Return_statement>(&body->statements[0].value);
  REQUIRE(ret_stmt != nullptr);
  CHECK(ret_stmt->kw_return.text == "return");
  auto const int_lit =
    std::get_if<kaleast::Int_literal_expression>(&ret_stmt->value.value);
...
TEST_CASE("Parser - parameters.kale parses successfully")
{
  auto file = std::ifstream{std::string{EXAMPLES_PATH} + "/parameters.kale"};
  auto binary_stream = kalelex::Istream_binary_stream{&file};
  auto char_stream = kalelex::Utf8_char_stream{&binary_stream};
  auto lexeme_stream = kalelex::Lexeme_stream{&char_stream};
  auto reader = kalelex::Lexeme_stream_reader{&lexeme_stream};
  ...
  auto const id_fn =
    std::get_if<kaleast::Fn_expression>(&id_decl.initializer.value);
  REQUIRE(id_fn != nullptr);
  auto const first_fn =
    std::get_if<kaleast::Fn_expression>(&first_decl.initializer.value);
  REQUIRE(first_fn != nullptr);
  auto const main_fn =
    std::get_if<kaleast::Fn_expression>(&main_decl.initializer.value);
  REQUIRE(main_fn != nullptr);
  // id: fn(x: Int32): Int32 => { return x; }
  REQUIRE(id_fn->parameters.size() == 1);
  CHECK(id_fn->parameters[0].name.text == "x");
  auto const id_param_type = std::get_if<kaleast::Identifier_expression>(
    &id_fn->parameters[0].type->value
  );
  REQUIRE(id_param_type != nullptr);
  CHECK(id_param_type->identifier.text == "Int32");
  auto const id_ret_type = std::get_if<kaleast::Identifier_expression>(
    &id_fn->return_type_specifier.type->value
  );
  REQUIRE(id_ret_type != nullptr);
  CHECK(id_ret_type->identifier.text == "Int32");
  auto const id_body =
    std::get_if<kaleast::Block_expression>(&id_fn->body->value);
  REQUIRE(id_body != nullptr);
  REQUIRE(id_body->statements.size() == 1);
  auto const id_ret =
    std::get_if<kaleast::Return_statement>(&id_body->statements[0].value);
  REQUIRE(id_ret != nullptr);
  auto const id_ret_val =
    std::get_if<kaleast::Identifier_expression>(&id_ret->value.value);
  ...
  auto const first_param0_type = std::get_if<kaleast::Identifier_expression>(
  ...
  auto const first_param1_type = std::get_if<kaleast::Identifier_expression>(
    &first_fn->parameters[1].type->value
  );
  REQUIRE(first_param1_type != nullptr);
  CHECK(first_param1_type->identifier.text == "Int32");
  auto const first_ret_type = std::get_if<kaleast::Identifier_expression>(
  ...
  auto const first_body =
    std::get_if<kaleast::Block_expression>(&first_fn->body->value);
  REQUIRE(first_body != nullptr);
  REQUIRE(first_body->statements.size() == 1);
  auto const first_ret =
    std::get_if<kaleast::Return_statement>(&first_body->statements[0].value);
  REQUIRE(first_ret != nullptr);
  auto const first_ret_val =
    std::get_if<kaleast::Identifier_expression>(&first_ret->value.value);
  REQUIRE(first_ret_val != nullptr);
  CHECK(first_ret_val->identifier.text == "x");
  // main: fn(): Int32 => { return first(id(42), 0); }
  auto const main_ret_type = std::get_if<kaleast::Identifier_expression>(
  ...
  auto const main_body =
    std::get_if<kaleast::Block_expression>(&main_fn->body->value);
  REQUIRE(main_body != nullptr);
  REQUIRE(main_body->statements.size() == 1);
  auto const main_ret =
    std::get_if<kaleast::Return_statement>(&main_body->statements[0].value);
  REQUIRE(main_ret != nullptr);
  // first(id(42), 0) — outer call
  auto const outer_call =
    std::get_if<kaleast::Call_expression>(&main_ret->value.value);
  REQUIRE(outer_call != nullptr);
  auto const outer_callee =
    std::get_if<kaleast::Identifier_expression>(&outer_call->callee->value);
  ...
  auto const inner_call =
    std::get_if<kaleast::Call_expression>(&outer_call->arguments[0].value);
  REQUIRE(inner_call != nullptr);
  auto const inner_callee =
    std::get_if<kaleast::Identifier_expression>(&inner_call->callee->value);
  REQUIRE(inner_callee != nullptr);
  CHECK(inner_callee->identifier.text == "id");
  REQUIRE(inner_call->arguments.size() == 1);
  auto const inner_arg = std::get_if<kaleast::Int_literal_expression>(
    &inner_call->arguments[0].value
  );
  REQUIRE(inner_arg != nullptr);
  CHECK(inner_arg->literal.text == "42");
  auto const outer_arg1 = std::get_if<kaleast::Int_literal_expression>(
...
TEST_CASE("Parser - call_expression.kale parses successfully")
{
  auto file =
    std::ifstream{std::string{EXAMPLES_PATH} + "/call_expression.kale"};
  auto binary_stream = kalelex::Istream_binary_stream{&file};
  auto char_stream = kalelex::Utf8_char_stream{&binary_stream};
  auto lexeme_stream = kalelex::Lexeme_stream{&char_stream};
  auto reader = kalelex::Lexeme_stream_reader{&lexeme_stream};
  ...
  CHECK(main_decl.name.text == "main");
  auto const foo_fn =
    std::get_if<kaleast::Fn_expression>(&foo_decl.initializer.value);
  ...
  auto const foo_body =
    std::get_if<kaleast::Block_expression>(&foo_fn->body->value);
  REQUIRE(foo_body != nullptr);
  REQUIRE(foo_body->statements.size() == 1);
  auto const foo_ret =
    std::get_if<kaleast::Return_statement>(&foo_body->statements[0].value);
  REQUIRE(foo_ret != nullptr);
  auto const foo_call =
    std::get_if<kaleast::Call_expression>(&foo_ret->value.value);
  REQUIRE(foo_call != nullptr);
  CHECK(foo_call->lparen.text == "(");
  CHECK(foo_call->rparen.text == ")");
  auto const foo_paren =
    std::get_if<kaleast::Paren_expression>(&foo_call->callee->value);
  REQUIRE(foo_paren != nullptr);
  CHECK(
    std::get_if<kaleast::Fn_expression>(&foo_paren->inner->value) != nullptr
  );
  ...
  auto const callee =
    std::get_if<kaleast::Identifier_expression>(&main_call->callee->value);
...
TEST_CASE("parse_let_statement - function declaration")
{
  ...
  CHECK(stmt.eq.text == "=");
  auto const fn = std::get_if<kaleast::Fn_expression>(&stmt.initializer.value);
  REQUIRE(fn != nullptr);
  CHECK(fn->kw_fn.text == "fn");
  REQUIRE(fn->body != nullptr);
  auto const body = std::get_if<kaleast::Block_expression>(&fn->body->value);
...
TEST_CASE("parse_let_statement")
{
  ...
  CHECK(stmt.eq.text == "=");
  auto const lit =
    std::get_if<kaleast::Int_literal_expression>(&stmt.initializer.value);
  REQUIRE(lit != nullptr);
...
TEST_CASE("parse_return_statement")
{
  ...
  auto const lit =
    std::get_if<kaleast::Int_literal_expression>(&stmt.value.value);
  REQUIRE(lit != nullptr);
...
TEST_CASE("parse_expression_statement")
{
  ...
  auto const id =
    std::get_if<kaleast::Identifier_expression>(&stmt.expression.value);
  REQUIRE(id != nullptr);
...
TEST_CASE("parse_block_expression")
{
  ...
  CHECK(
    std::get_if<kaleast::Return_statement>(&block.statements[0].value) !=
    nullptr
  );
  CHECK(
    std::get_if<kaleast::Let_statement>(&block.statements[1].value) != nullptr
  );
...
TEST_CASE("parse_block_expression - tail expression")
{
  ...
  auto const lit =
    std::get_if<kaleast::Int_literal_expression>(&block.tail->value);
  REQUIRE(lit != nullptr);
  CHECK(lit->literal.text == "42");
}
...
TEST_CASE("parse_block_expression - statements then tail")
{
  ...
  CHECK(
    std::get_if<kaleast::Let_statement>(&block.statements[0].value) != nullptr
  );
  REQUIRE(block.tail != nullptr);
  auto const bin = std::get_if<kaleast::Binary_expression>(&block.tail->value);
  REQUIRE(bin != nullptr);
  CHECK(bin->op.text == "+");
}
...
TEST_CASE("parse_expression - block as primary expression")
{
  ...
  auto const block = std::get_if<kaleast::Block_expression>(&expr->value);
  REQUIRE(block != nullptr);
  REQUIRE(block->tail != nullptr);
  CHECK(
    std::get_if<kaleast::Binary_expression>(&block->tail->value) != nullptr
  );
}
...
TEST_CASE("parse_expression - nested blocks")
{
  ...
  auto const b0 = std::get_if<kaleast::Block_expression>(&expr->value);
  REQUIRE(b0 != nullptr);
  CHECK(b0->statements.empty());
  REQUIRE(b0->tail != nullptr);
  auto const b1 = std::get_if<kaleast::Block_expression>(&b0->tail->value);
  REQUIRE(b1 != nullptr);
  CHECK(b1->statements.empty());
  REQUIRE(b1->tail != nullptr);
  auto const b2 = std::get_if<kaleast::Block_expression>(&b1->tail->value);
  REQUIRE(b2 != nullptr);
  CHECK(b2->statements.empty());
  REQUIRE(b2->tail != nullptr);
  auto const b3 = std::get_if<kaleast::Block_expression>(&b2->tail->value);
  REQUIRE(b3 != nullptr);
...
TEST_CASE("parse_block_expression - fn body parses tail syntactically")
{
  ...
  auto const lit =
    std::get_if<kaleast::Int_literal_expression>(&body->tail->value);
  REQUIRE(lit != nullptr);
  CHECK(lit->literal.text == "42");
}
...
TEST_CASE("parse_fn_expression")
{
  ...
  CHECK(fn.rparen.text == ")");
  CHECK(fn.return_type_specifier.colon.text == ":");
  auto const ret_type = std::get_if<kaleast::Identifier_expression>(
    &fn.return_type_specifier.type->value
  );
...
TEST_CASE("parse_paren_expression")
{
  ...
  auto const inner =
    std::get_if<kaleast::Int_literal_expression>(&expr.inner->value);
...
TEST_CASE("parse_paren_expression - nested")
{
  ...
  CHECK(expr.lparen.text == "(");
  auto const inner =
    std::get_if<kaleast::Paren_expression>(&expr.inner->value);
  REQUIRE(inner != nullptr);
  auto const id =
    std::get_if<kaleast::Identifier_expression>(&inner->inner->value);
  REQUIRE(id != nullptr);
...
TEST_CASE("parse_primary_expression - dispatches to int literal")
{
  ...
  CHECK(std::get_if<kaleast::Int_literal_expression>(&expr->value) != nullptr);
}
...
TEST_CASE("parse_primary_expression - dispatches to identifier")
{
  auto fixture = Parse_fixture{"x"};
  auto const expr = fixture.parser.parse_primary_expression();
  CHECK(std::get_if<kaleast::Identifier_expression>(&expr->value) != nullptr);
}
...
TEST_CASE("parse_primary_expression - dispatches to fn")
{
  ...
  auto const expr = fixture.parser.parse_primary_expression();
  CHECK(std::get_if<kaleast::Fn_expression>(&expr->value) != nullptr);
}
...
TEST_CASE("parse_primary_expression - dispatches to paren")
{
  auto fixture = Parse_fixture{"(42)"};
  auto const expr = fixture.parser.parse_primary_expression();
  CHECK(std::get_if<kaleast::Paren_expression>(&expr->value) != nullptr);
}
...
TEST_CASE("parse_expression - simple binary expression")
{
  auto fixture = Parse_fixture{"1 + 2"};
  auto const expr = fixture.parser.parse_expression();
  auto const bin = std::get_if<kaleast::Binary_expression>(&expr->value);
  REQUIRE(bin != nullptr);
  auto const left =
    std::get_if<kaleast::Int_literal_expression>(&bin->left->value);
  REQUIRE(left != nullptr);
  CHECK(left->literal.text == "1");
  CHECK(bin->op.text == "+");
  auto const right =
    std::get_if<kaleast::Int_literal_expression>(&bin->right->value);
...
TEST_CASE("parse_expression - multiplicative before additive")
{
  ...
  auto const expr = fixture.parser.parse_expression();
  auto const add = std::get_if<kaleast::Binary_expression>(&expr->value);
  REQUIRE(add != nullptr);
  CHECK(add->op.text == "+");
  auto const mul = std::get_if<kaleast::Binary_expression>(&add->right->value);
  REQUIRE(mul != nullptr);
  CHECK(mul->op.text == "*");
  auto const two =
    std::get_if<kaleast::Int_literal_expression>(&mul->left->value);
  REQUIRE(two != nullptr);
  CHECK(two->literal.text == "2");
  auto const three =
    std::get_if<kaleast::Int_literal_expression>(&mul->right->value);
...
TEST_CASE("parse_expression - left associativity")
{
  ...
  auto fixture = Parse_fixture{"1 - 2 - 3"};
  auto const expr = fixture.parser.parse_expression();
  auto const outer = std::get_if<kaleast::Binary_expression>(&expr->value);
  REQUIRE(outer != nullptr);
  CHECK(outer->op.text == "-");
  auto const inner =
    std::get_if<kaleast::Binary_expression>(&outer->left->value);
  REQUIRE(inner != nullptr);
  CHECK(inner->op.text == "-");
  auto const three =
    std::get_if<kaleast::Int_literal_expression>(&outer->right->value);
  REQUIRE(three != nullptr);
  CHECK(three->literal.text == "3");
}
...
TEST_CASE("parse_expression - all operators")
{
  ...
  auto const expr = fixture.parser.parse_expression();
  // outer: (-f() + +2) - (...)
  auto const sub = std::get_if<kaleast::Binary_expression>(&expr->value);
  REQUIRE(sub != nullptr);
  CHECK(sub->op.text == "-");
  // left of -: -f() + +2
  auto const add = std::get_if<kaleast::Binary_expression>(&sub->left->value);
  REQUIRE(add != nullptr);
  CHECK(add->op.text == "+");
  // left of +: -f()
  auto const unary_minus =
    std::get_if<kaleast::Prefix_expression>(&add->left->value);
  REQUIRE(unary_minus != nullptr);
  CHECK(unary_minus->op.text == "-");
  auto const call =
    std::get_if<kaleast::Call_expression>(&unary_minus->operand->value);
  REQUIRE(call != nullptr);
  auto const callee =
    std::get_if<kaleast::Identifier_expression>(&call->callee->value);
  REQUIRE(callee != nullptr);
  CHECK(callee->identifier.text == "f");
  // right of +: +2
  auto const unary_plus =
    std::get_if<kaleast::Prefix_expression>(&add->right->value);
  REQUIRE(unary_plus != nullptr);
  CHECK(unary_plus->op.text == "+");
  // right of -: ((3 * 4) / 5) % 6
  auto const mod = std::get_if<kaleast::Binary_expression>(&sub->right->value);
  REQUIRE(mod != nullptr);
  CHECK(mod->op.text == "%");
  // left of %: (3 * 4) / 5
  auto const div = std::get_if<kaleast::Binary_expression>(&mod->left->value);
  REQUIRE(div != nullptr);
  CHECK(div->op.text == "/");
  // left of /: 3 * 4
  auto const mul = std::get_if<kaleast::Binary_expression>(&div->left->value);
  REQUIRE(mul != nullptr);
  CHECK(mul->op.text == "*");
}
...
TEST_CASE("parse_expression - call binds tighter than binary op")
{
  ...
  REQUIRE(add != nullptr);
  CHECK(add->op.text == "+");
  CHECK(std::get_if<kaleast::Call_expression>(&add->left->value) != nullptr);
}
...
TEST_CASE("parse_statement - dispatches to let")
{
  ...
  CHECK(std::get_if<kaleast::Let_statement>(&stmt.value) != nullptr);
}
...
TEST_CASE("parse_statement - dispatches to return")
{
  ...
  auto const stmt = fixture.parser.parse_statement();
  CHECK(std::get_if<kaleast::Return_statement>(&stmt.value) != nullptr);
}
...
TEST_CASE("parse_statement - dispatches to expression statement")
{
  auto fixture = Parse_fixture{"x;"};
  auto const stmt = fixture.parser.parse_statement();
  CHECK(std::get_if<kaleast::Expression_statement>(&stmt.value) != nullptr);
}
...
TEST_CASE("parse_expression - unary minus: call binds tighter")
{
  ...
  auto const expr = fixture.parser.parse_expression();
  auto const unary = std::get_if<kaleast::Prefix_expression>(&expr->value);
  REQUIRE(unary != nullptr);
  CHECK(unary->op.text == "-");
  CHECK(
    std::get_if<kaleast::Call_expression>(&unary->operand->value) != nullptr
  );
}
...
TEST_CASE("parse_fn_expression - array parameter")
{
  ...
  auto const param_type =
    std::get_if<kaleast::Index_expression>(&fn.parameters[0].type->value);
  REQUIRE(param_type != nullptr);
  auto const elem =
    std::get_if<kaleast::Identifier_expression>(&param_type->operand->value);
  REQUIRE(elem != nullptr);
  CHECK(elem->identifier.text == "Int32");
  auto const size =
    std::get_if<kaleast::Int_literal_expression>(&param_type->index->value);
...
TEST_CASE("parse_expression - index: simple")
{
  ...
  auto const expr = fixture.parser.parse_expression();
  auto const idx = std::get_if<kaleast::Index_expression>(&expr->value);
  REQUIRE(idx != nullptr);
  auto const operand =
    std::get_if<kaleast::Identifier_expression>(&idx->operand->value);
  REQUIRE(operand != nullptr);
  CHECK(operand->identifier.text == "arr");
  CHECK(idx->lbracket.text == "[");
  auto const index =
    std::get_if<kaleast::Int_literal_expression>(&idx->index->value);
...
TEST_CASE("parse_expression - index: chained")
{
  ...
  auto const expr = fixture.parser.parse_expression();
  auto const outer = std::get_if<kaleast::Index_expression>(&expr->value);
  REQUIRE(outer != nullptr);
  auto const outer_index =
    std::get_if<kaleast::Int_literal_expression>(&outer->index->value);
  REQUIRE(outer_index != nullptr);
  CHECK(outer_index->literal.text == "1");
  auto const inner =
    std::get_if<kaleast::Index_expression>(&outer->operand->value);
  REQUIRE(inner != nullptr);
  auto const inner_operand =
    std::get_if<kaleast::Identifier_expression>(&inner->operand->value);
  REQUIRE(inner_operand != nullptr);
  CHECK(inner_operand->identifier.text == "arr");
  auto const inner_index =
    std::get_if<kaleast::Int_literal_expression>(&inner->index->value);
...
TEST_CASE("parse_expression - index: expression index")
{
  ...
  REQUIRE(idx != nullptr);
  auto const index =
    std::get_if<kaleast::Binary_expression>(&idx->index->value);
  REQUIRE(index != nullptr);
  CHECK(index->op.text == "+");
}
...
TEST_CASE("parse_expression - index: binds tighter than binary op")
{
  ...
  REQUIRE(add != nullptr);
  CHECK(add->op.text == "+");
  CHECK(std::get_if<kaleast::Index_expression>(&add->left->value) != nullptr);
}
...
TEST_CASE("parse_expression - index: call then index")
{
  ...
  REQUIRE(idx != nullptr);
  CHECK(
    std::get_if<kaleast::Call_expression>(&idx->operand->value) != nullptr
  );
}
...
TEST_CASE("parse_expression - dereference")
{
  ...
  auto const expr = fixture.parser.parse_expression();
  auto const postfix = std::get_if<kaleast::Postfix_expression>(&expr->value);
  REQUIRE(postfix != nullptr);
  CHECK(postfix->op.text == "^");
  auto const operand =
    std::get_if<kaleast::Identifier_expression>(&postfix->operand->value);
  REQUIRE(operand != nullptr);
...
TEST_CASE("parse_expression - dereference: index then deref")
{
  ...
  CHECK(postfix->op.text == "^");
  CHECK(
    std::get_if<kaleast::Index_expression>(&postfix->operand->value) != nullptr
  );
}
...
TEST_CASE("parse_expression - dereference: in binary expression")
{
  ...
  CHECK(add->op.text == "+");
  auto const left =
    std::get_if<kaleast::Postfix_expression>(&add->left->value);
  REQUIRE(left != nullptr);
  CHECK(left->op.text == "^");
  auto const right =
    std::get_if<kaleast::Postfix_expression>(&add->right->value);
  REQUIRE(right != nullptr);
  CHECK(right->op.text == "^");
}
...
TEST_CASE("parse_expression - address-of")
{
  ...
  CHECK(unary->op.text == "&");
  auto const operand =
    std::get_if<kaleast::Identifier_expression>(&unary->operand->value);
  REQUIRE(operand != nullptr);
...
TEST_CASE("parse_expression - address-of: postfix binds tighter")
{
  ...
  CHECK(unary->op.text == "&");
  CHECK(
    std::get_if<kaleast::Index_expression>(&unary->operand->value) != nullptr
  );
}
...
TEST_CASE("parse_expression - address-of: in binary expression")
{
  ...
  REQUIRE(add != nullptr);
  CHECK(add->op.text == "+");
  auto const left = std::get_if<kaleast::Prefix_expression>(&add->left->value);
  REQUIRE(left != nullptr);
  CHECK(left->op.text == "&");
  auto const right =
    std::get_if<kaleast::Prefix_expression>(&add->right->value);
...
TEST_CASE("parse_expression - if: simple")
{
  ...
  auto const expr = fixture.parser.parse_expression();
  auto const if_expr = std::get_if<kaleast::If_expression>(&expr->value);
  REQUIRE(if_expr != nullptr);
  CHECK(if_expr->kw_if.text == "if");
  auto const cond =
    std::get_if<kaleast::Identifier_expression>(&if_expr->condition->value);
...
TEST_CASE("parse_expression - else if chain")
{
  ...
  auto const cond = std::get_if<kaleast::Identifier_expression>(
...
TEST_CASE("parse_expression - additive before comparison")
{
  ...
  auto const expr = fixture.parser.parse_expression();
  auto const lt = std::get_if<kaleast::Binary_expression>(&expr->value);
  REQUIRE(lt != nullptr);
  CHECK(lt->op.text == "<");
  auto const left = std::get_if<kaleast::Binary_expression>(&lt->left->value);
  REQUIRE(left != nullptr);
  CHECK(left->op.text == "+");
  auto const right =
    std::get_if<kaleast::Binary_expression>(&lt->right->value);
  REQUIRE(right != nullptr);
  CHECK(right->op.text == "+");
}
...
TEST_CASE("parse_expression - comparison before equality")
{
  ...
  auto fixture = Parse_fixture{"1 < 2 == 3 > 4"};
  auto const expr = fixture.parser.parse_expression();
  auto const eq = std::get_if<kaleast::Binary_expression>(&expr->value);
  REQUIRE(eq != nullptr);
  CHECK(eq->op.text == "==");
  auto const left = std::get_if<kaleast::Binary_expression>(&eq->left->value);
  REQUIRE(left != nullptr);
  CHECK(left->op.text == "<");
  auto const right =
    std::get_if<kaleast::Binary_expression>(&eq->right->value);
  REQUIRE(right != nullptr);
  CHECK(right->op.text == ">");
}
...
TEST_CASE("parse_expression - assign: simple")
{
  ...
  CHECK(bin->op.text == "=");
  auto const left =
    std::get_if<kaleast::Identifier_expression>(&bin->left->value);
  REQUIRE(left != nullptr);
...
TEST_CASE("parse_expression - assign: right-associative")
{
  ...
  CHECK(outer->op.text == "=");
  auto const left =
    std::get_if<kaleast::Identifier_expression>(&outer->left->value);
  REQUIRE(left != nullptr);
  CHECK(left->identifier.text == "a");
  auto const inner =
    std::get_if<kaleast::Binary_expression>(&outer->right->value);
  REQUIRE(inner != nullptr);
  CHECK(inner->op.text == "=");
  auto const inner_left =
    std::get_if<kaleast::Identifier_expression>(&inner->left->value);
...
TEST_CASE("parse_expression - assign: lower precedence than equality")
{
  ...
  auto const expr = fixture.parser.parse_expression();
  auto const assign = std::get_if<kaleast::Binary_expression>(&expr->value);
  REQUIRE(assign != nullptr);
  CHECK(assign->op.text == "=");
  auto const rhs =
    std::get_if<kaleast::Binary_expression>(&assign->right->value);
  REQUIRE(rhs != nullptr);
  CHECK(rhs->op.text == "==");
}
...
TEST_CASE("parse_statement - while: simple")
{
  ...
  auto const stmt = fixture.parser.parse_statement();
  auto const while_stmt = std::get_if<kaleast::While_statement>(&stmt.value);
  REQUIRE(while_stmt != nullptr);
  CHECK(while_stmt->kw_while.text == "while");
  auto const cond =
    std::get_if<kaleast::Identifier_expression>(&while_stmt->condition->value);
  REQUIRE(cond != nullptr);
  CHECK(cond->identifier.text == "x");
...
TEST_CASE("parse_statement - while: with body")
{
  ...
  REQUIRE(while_stmt != nullptr);
  auto const cond =
    std::get_if<kaleast::Binary_expression>(&while_stmt->condition->value);
  REQUIRE(cond != nullptr);
  CHECK(cond->op.text == ">");
...
TEST_CASE("parse_let_statement - mut")
{
  ...
  auto const stmt = fixture.parser.parse_statement();
  auto const let_stmt = std::get_if<kaleast::Let_statement>(&stmt.value);
...
TEST_CASE("Parser - quicksort.kale parses successfully")
{
  auto file = std::ifstream{std::string{EXAMPLES_PATH} + "/quicksort.kale"};
  auto binary_stream = kalelex::Istream_binary_stream{&file};
  auto char_stream = kalelex::Utf8_char_stream{&binary_stream};
  auto lexeme_stream = kalelex::Lexeme_stream{&char_stream};
  auto reader = kalelex::Lexeme_stream_reader{&lexeme_stream};
...
TEST_CASE("parse_expression - prefix bracket unsized array type")
{
  ...
  auto const expr = fixture.parser.parse_expression();
  auto const prefix =
    std::get_if<kaleast::Prefix_bracket_expression>(&expr->value);
  REQUIRE(prefix != nullptr);
  CHECK(prefix->lbracket.text == "[");
  CHECK(prefix->size == nullptr);
  CHECK(prefix->rbracket.text == "]");
  auto const operand =
    std::get_if<kaleast::Identifier_expression>(&prefix->operand->value);
  REQUIRE(operand != nullptr);
...
TEST_CASE("parse_expression - prefix bracket sized array type")
{
  ...
  auto const size =
    std::get_if<kaleast::Int_literal_expression>(&prefix->size->value);
  REQUIRE(size != nullptr);
  CHECK(size->literal.text == "4");
  CHECK(prefix->rbracket.text == "]");
  auto const operand =
...
TEST_CASE("parse_expression - pointer to unsized array type")
{
  ...
  CHECK(unary->op.text == "^");
  auto const prefix =
    std::get_if<kaleast::Prefix_bracket_expression>(&unary->operand->value);
  REQUIRE(prefix != nullptr);
  CHECK(prefix->size == nullptr);
  auto const operand =
    std::get_if<kaleast::Identifier_expression>(&prefix->operand->value);
...
TEST_CASE("parse_expression - ^mut prefix")
{
  ...
  CHECK(unary->op.token == kalelex::Token::caret_mut);
  auto const operand =
    std::get_if<kaleast::Identifier_expression>(&unary->operand->value);
  REQUIRE(operand != nullptr);
  CHECK(operand->identifier.text == "Int32");
