...
namespace benson::ir
{
      ...
      : _type_pool{type_pool}
  {
    ...
    _unary_overloads[ast::Operator::unary_minus].push_back(
    ...
    _unary_overloads[ast::Operator::pointer_to].push_back(
      std::make_unique<Pointer_to>(_type_pool)
    );
    _unary_overloads[ast::Operator::pointer_to_mut].push_back(
      std::make_unique<Pointer_to_mut>(_type_pool)
    );
    _unary_overloads[ast::Operator::dereference].push_back(
      std::make_unique<Dereference>()
    );
    _binary_overloads[ast::Operator::add].push_back(
    ...
    _binary_overloads[ast::Operator::subtract].push_back(
    ...
    _binary_overloads[ast::Operator::multiply].push_back(
    ...
    _binary_overloads[ast::Operator::divide].push_back(
    ...
    _binary_overloads[ast::Operator::modulo].push_back(
    ...
    _binary_overloads[ast::Operator::equal].push_back(
    ...
    _binary_overloads[ast::Operator::not_equal].push_back(
    ...
    _binary_overloads[ast::Operator::less].push_back(
    ...
    _binary_overloads[ast::Operator::less_eq].push_back(
    ...
    _binary_overloads[ast::Operator::greater].push_back(
    ...
    _binary_overloads[ast::Operator::greater_eq].push_back(
  ...
  Symbol *
  Compilation_context::try_lookup_identifier(Lexeme const &identifier)
  {
    return _symbol_table.lookup(identifier.text);
  }

  Symbol *
  Compilation_context::lookup_identifier(Lexeme const &identifier)
  {
    ...
    catch (Not_a_constant_error const &e)
    {
      _diagnostics.push_back(
        Diagnostic{
          ...
          .location = e.expression_location.value_or(ast::span_of(expr)),
          .notes = {},
        }
      );
      throw Compilation_error{};
  ...
  Operand
  Compilation_context::compile_expression(ast::Expression const &expr)
  {
    try
    {
    ...
    catch (Not_a_constant_error &e)
    {
      ...
      {
        e.expression_location = ast::span_of(expr);
      }
      throw;
    }
  }
  ...
  Operand Compilation_context::compile_expression(
    ast::Recurse_expression const &expr
  )
  {
    if (is_top_level())
    {
  ...
  Operand Compilation_context::compile_expression(
    ast::Paren_expression const &expr
  )
  {
  ...
  Operand Compilation_context::compile_expression(
    ast::Prefix_expression const &expr
  )
  {
    auto const op = ast::get_prefix_operator(expr.op.token);
    assert(op.has_value());
    // Special case: unary minus before int literal expression
    if (*op == ast::Operator::unary_minus &&
        std::holds_alternative<ast::Int_literal_expression>(
          expr.operand->value
        ))
    {
      auto const &literal =
  ...
  Operand Compilation_context::compile_expression(
    ast::Postfix_expression const &expr
  )
  {
  ...
  Operand
  Compilation_context::compile_expression(ast::Index_expression const &)
  {
  ...
  Operand Compilation_context::compile_expression(
    ast::Block_expression const &expr
  )
  {
  ...
  )
  {
    ...
    catch (Not_a_constant_error const &e)
    {
      _diagnostics.push_back(
        Diagnostic{
          .message = "expression is not a compile-time constant",
          .location =
            e.expression_location.value_or(ast::span_of(stmt.initializer)),
          .notes = {Diagnostic_note{
            .message = "required by this top-level binding",
            .location = ast::span_of(stmt),
          }},
        }
      );
      throw Compilation_error{};
    ...
    catch (Fuel_exhausted_error const &)
    {
      _diagnostics.push_back(
        Diagnostic{
          .message = "compile-time evaluation ran out of fuel",
          .location = ast::span_of(stmt.initializer),
          .notes = {Diagnostic_note{
            .message = "required by this top-level binding",
            .location = ast::span_of(stmt),
          }},
  ...
  void Compilation_context::compile_statement(
    ast::While_statement const &stmt
  )
  {
    emit_error(
  ...
  void Compilation_context::compile_statement(
    ast::Return_statement const &stmt
  )
  {
