...
`kaleparse` is the parser module.
...
This module consumes lexemes and produces the AST defined in [`kaleast`](./kaleast.md).

## Core abstractions

### `Parser`

Hand-written parser over `Lexeme_stream_reader`. Produces `Translation_unit`, `Expression`, and `Statement` values from `kaleast`.
...
- [`kaleast`](./kaleast.md) — data model produced by this module
